...
RSpec.describe Api::AssessmentsController, type: :controller do
  describe 'GET #show' do
    ...
    let!(:grouped_question) do
      create(:question, assessment:, question_group: group, text: 'Grouped Q1', position: 1)
    end
    let!(:standalone_question) { create(:question, assessment:, text: 'Standalone Q1', position: 2) }
    ...
    it 'returns the assessment with nested groups, questions, and answers' do
      ...
      expect(json['question_groups'][0]['questions'][0]['text']).to eq('Grouped Q1')
      expect(json['question_groups'][0]['questions'][0]['answers'][0]['text']).to eq('Low')

      # Standalone questions
      expect(json['questions'].size).to eq(1)
      expect(json['questions'][0]['text']).to eq('Standalone Q1')
