...
ActiveAdmin.register Event do
  ...
  member_action :copy, method: :get do
    duplicated_event = resource.duplicate

    if duplicated_event.save(validate: false)
      redirect_to edit_admin_event_path(duplicated_event),
                  notice: 'Event copied successfully. Please set the dates and review the details.'
    else
      redirect_to admin_events_path,
                  alert: "Failed to copy event: #{duplicated_event.errors.full_messages.join(', ')}"
    end
  end
  ...
  index do
    ...
    actions defaults: false do |event|
      ...
      text_node ' | '
      item link_to('Copiar', copy_admin_event_path(event))
    end
  end
...
"
    ...
    div id: 'hr-form-overlay',
        style: 'display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;' do
